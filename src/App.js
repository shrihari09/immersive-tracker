        ...
        <div style={{padding:"24px 32px"}}>
          ...
          {viewMode==="grid"?(
            ...
            <div style={{background:T.surface,border:`1px solid ${T.border}`,borderRadius:12,overflow:"hidden",boxShadow:T.shadow}}>
              ...
              {filtered.map((p,i)=>{ const done=p.tasks.filter(t=>t.status==="Done").length, pct=p.tasks.length>0?Math.round((done/p.tasks.length)*100):0, sc=statusColor[p.status]||T.text3;
  ...
  return (
    <div style={{padding:"18px 0"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))",gap:14}}>
        {SECTIONS.map(section=>{const st=filtered.filter(t=>t.section===section);const allSec=tasks.filter(t=>t.section===section);const done=allSec.filter(t=>t.status==="Done").length;const pct=allSec.length>0?Math.round((done/allSec.length)*100):0;if(!allSec.length) return null;
          return(<div key={section} style={{background:T.surface,border:`1px solid ${T.border}`,borderRadius:11,overflow:"hidden",boxShadow:T.shadow}}>
            ...
            <div style={{padding:"7px 7px 5px"}}>
              ...
              {st.map(task=>{const cfg=STATUS[task.status]||STATUS["Not Started"];const ac2=teamColors[task.artist]||T.text3;const pct=task.manDays>0?Math.round((task.daysUsed/task.manDays)*100):0;const od=task.endDate<TODAY_STR&&task.status!=="Done";
    ...
    <div style={{padding:"22px 0"}}>
      ...
      {cad.length===0?<div style={{textAlign:"center",padding:"50px 20px",color:T.text3,background:T.surface,borderRadius:12,border:`1px solid ${T.border}`}}><div style={{fontSize:30,marginBottom:10}}>📂</div><div style={{fontSize:13,color:T.text2,fontWeight:500,marginBottom:4}}>No CAD files added yet</div><div style={{fontSize:11}}>Click "Add CAD File" to start tracking files from the client.</div></div>:(
        <div style={{background:T.surface,border:`1px solid ${T.border}`,borderRadius:12,overflow:"hidden",boxShadow:T.shadow}}>
          ...
          {cad.map((item,idx)=>{const cs=CAD_STATUS_STYLE[item.status]||CAD_STATUS_STYLE["Awaiting from Client"];const ac2=teamColors[item.assignee]||T.text3;
